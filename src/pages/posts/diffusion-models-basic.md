...
The original paper covers everything in detail—this post merely provides a casual summary. ps: [Paper link](https://r2.nkd.red/papers/diffusion-model/2006.11239.pdf)
...
1. Decompose the log-likelihood:
   Start with the log-likelihood of the data $\log p_\theta\left(\mathbf{x}_0\right)$ and apply the chain rule of probability:
...
2. Apply Jensen's inequality:
   Because the log function is concave, Jensen's inequality allows moving the log outside the integral:
   $$
   ...
   $$
3. Expectation form:
   Rewrite the expression as an expectation:
   $$
