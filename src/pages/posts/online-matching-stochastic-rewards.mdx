...
_For any thresholds $\theta_{-u}$ of offline vertices other than $u$ and the corresponding $ \ell*u^{\infty} $:*
...
Fix $\theta_{-u}$ and let $\theta_u = \infty$ (background world for some fantasy description).
...
Define the cumulative distribution function that tracks which online vertices from $S$ have been matched by the time $u$'s load reaches $\theta$:

$$
M_S(\ell) := p \cdot |\{ v \in S \mid \text{ the event } v \text{ is matched happened when } u\text{'s load} \leq \ell \}|
$$
...
$$
\varphi_S(\ell) := \frac{d\mu_S}{d\ell} (\ell),
\quad
M_S(\ell) = \int_0^{\ell} \varphi_S(z)\, \d z,
\quad
\int_0^{\ell_u^{\infty}} \varphi_S(z)\, \d z = p \cdot R.
$$

Note that $M_S(\ell^\infty) \geq p_u(S)$ (because $p_u(S) = \min\{\sum_{v \in S} p, 1\}$, and in the "background world" where $u$ is always available, $M_S(\ell^\infty) = p \cdot |S| \geq p_u(S)$).

Intuitively, $\varphi_S(\ell)$ is the "load density" from $S$ at load $\ell$ (it describes the rate at which online vertices from $S$ are being matched when $u$'s load is at $\ell$). Note that $\ell_u^{\infty}$ is the total load on $u$ when $\theta_u = \infty$ in this background world.

When a micro event happened, if $u$ load is $\ell_u$, then the contribution to
...
&= e^{-\ell_u^{\infty}} \cdot \int_0^{\ell_u^{\infty}} \left( 1 - f(\ell_v) \right) \d\mu_S(\theta_u) \\
...
### Definition of Extended World

For each threshold $\theta_u$, define another world where $\theta_u$ be the threshold of $u$ and all other thresholds $\theta_{-u}$ remain the same.

Define cumulative distribution function on this world:

$$
M_S^{(\theta_u)}(\ell) := p \cdot |\{ v \in S \mid \text{ the event } v \text{ is matched happened when } u\text{'s load} \leq \ell \}|
$$

$$
\mu_S^{(\theta_u)}((a,b]) := M_S^{(\theta_u)}(b) - M_S^{(\theta_u)}(a), \quad
\varphi_S^{(\theta_u)}(z) := \frac{d\mu_S^{(\theta_u)}}{dz}(z),
$$

where $M_S^{(\theta_u)}(\cdot)$ tracks the mass of online vertices $v$ from $S$ matched when $u$'s load is at most $\cdot$ given $\theta_u$.
...
In the "$\theta = \infty$ background world", we decompose $S$ into:
...
The alternating path "splits off" this portion of the flow and reroutes it to other offline vertices or to higher load levels.

Meanwhile, $S_2$ remains _equivalent_ unchanged (possibly only with some vertices from $S_1$ replacing vertices from $S_2$ in their original positions, i.e., identities are swapped).
...
Let the "background world" be $G(\infty, \theta_{-u})$ and the "$\theta_u$-world" be $G(\theta_u, \theta_{-u})$.
