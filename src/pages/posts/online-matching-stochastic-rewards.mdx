...
Within each online node $v_j$, the random seat selection spreads its "unit of effort" uniformly
over a large available set $S_j$, so the per-offline-node hit probability is
approximately $1/R_j$, and the **average** Phase-A hazard per offline node accumulates to $k/n$.
Survival is the product of "not hit" across independent Poisson draws,
giving $e^{-k/n}$ (or, more finely for offline node $u_i$, $e^{-\min\{i,k\}/n}$).
...
- **Concentration:** $H_U^{(A)}$ is a bounded-difference functional of the per-online-node randomness
  with online-node-wise Lipschitz constants $O(1/R_j)$ and $\sum O(1/R_j)=O(1)$,
