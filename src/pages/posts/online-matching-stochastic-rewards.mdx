...
$$
M_S(\theta) := p \cdot |\{ v \in S \mid \text{ the event } v \text{ is matched happened when } u\text{'s load} \leq \theta \}|
$$
...
$$
\varphi_S(\theta) := \frac{d\mu_S}{d\theta} (\theta),
\quad
M_S(\theta) = \int_0^{\theta} \varphi_S(z)\, dz,
\quad
\int_0^{\ell_u^{\infty}} \varphi_S(z)\, dz = p \cdot R.
$$
...
Intuitively, $\varphi_S(\theta)$ is the "load density" from $S$ at threshold $\theta$ (it describes the rate at which online vertices from $S$ are being matched when $u$'s load is at $\theta$). Note that $\ell_u^{\infty}$ is the total load on $u$ when $\theta_u = \infty$ in this background world.
...
&\geq e^{-\ell_u^{\infty}} \cdot \int_0^{\ell_u^{\infty}} \left( 1 - f(\theta_u) \right) \cdot \varphi_S(\theta_u)\, d\theta_u
\end{aligned}
$$
...
$$
\begin{aligned}
& e^{-\ell_u^{\infty}} \cdot \int_0^{\ell_u^{\infty}} \left( 1 - f(\theta_u) \right) \cdot \varphi_S(\theta_u)\, d\theta_u \\
\geq & e^{-\ell_u^{\infty}} \cdot \left( 1 - f(\ell_u^{\infty}) \right) \cdot \int_0^{\ell_u^{\infty}}\varphi_S(\theta_u)\, d\theta_u \\
...
$$
M_S^{(\theta_u)}(\theta) := p \cdot |\{ v \in S \mid \text{ the event } v \text{ is matched happened when } u\text{'s load} \leq \theta \}|
$$
