...
Note that $M_S(\ell^\infty) \geq p_u(S)$ (because $p_u(S) = \min\{\sum_{v \in S} p, 1\}$, and in the "phantom world" where $u$ is always available, $M_S(\ell^\infty) = p \cdot |S| \geq p_u(S)$).
