...
However, this increment only occurs when vertex $u$ has not yet succeeded (i.e., when $\theta_u > \ell$), which happens with probability $\Pr[\theta_u > \ell] = e^{-\ell}$.
...
\mathbb{E}_{\theta_u}\!\left[\alpha_u \mid \theta_{-u}\right]
=
\int_{0}^{\ell_u^{\infty}} P[ \theta_u > \ell] f(\theta_u)\, d\theta_u
=
\int_{0}^{\ell_u^{\infty}} e^{-\theta_u}\, f(\theta_u)\, d\theta_u \, .
$$
...
$S$ be any feasible set of offline vertices that $S \subseteq N(u)$.
...
In each event, $\beta_v = p \cdot (1 - f(\ell_v))$ if matched, and $0$ otherwise. ($\ell_v$ means in this world, the load of the offline vertex that $v$ is matched to when this event happened. Doesn't have to be $u$)
